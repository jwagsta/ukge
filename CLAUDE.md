...
- `scripts/buildConstituencyContinuity.ts` - Build constituency continuity mapping from transition data (run with `npx tsx`)
...
**Wikipedia Integration**: `src/services/wikipedia.ts` provides a REST API client for Wikipedia summaries. `src/hooks/useWikipedia.ts` exposes `useConstituencyWikipedia` and `useElectionWikipedia` hooks. Constituency ID → Wikipedia article title mapping is in `public/data/wikipedia/constituencyMapping.json` (generated by `scripts/buildWikipediaMapping.ts`). Election article titles are deterministic (`{year}_United_Kingdom_general_election`). `ElectionInfoBar` shows election summaries below the header; `WikipediaSnippet` shows constituency summaries in collapsible sections within `ConstituencyPanel` and `MobileBottomSheet`.

**Historical Data Hook**: `src/hooks/useHistoricalData.ts` provides `useHistoricalData()` — shared hook used by both `ConstituencyPanel` and `MobileBottomSheet` to fetch historical election data for a selected constituency across all years. Returns `breakPoints: Set<number>` indicating indices where constituency chart lines should break due to boundary discontinuities (based on continuity data). Loads continuity JSON once at module level.

**Linked Views**: Hover/select in one visualization highlights in others via store state.

## Data Files

**Scope**: Great Britain (England, Scotland, Wales) for all years. Northern Ireland included for 2024 (shown as map inset; excluded from ternary plot).

**Elections**: `public/data/elections/{year}.json` - Years 1955-2024 (use `197402`/`197410` for Feb/Oct 1974)

**Wikipedia Mapping**: `public/data/wikipedia/constituencyMapping.json` - Maps constituency IDs (e.g., `EC_ALDERSHOT`) to Wikipedia article titles (e.g., `Aldershot_(UK_Parliament_constituency)`). Generated by `scripts/buildWikipediaMapping.ts`. Rebuild if new constituencies are added.

**Constituency Continuity**: `public/data/continuity/constituencyContinuity.json` - Lists constituency IDs that have >= 90% same-ID area overlap across each boundary transition. Generated by `scripts/buildConstituencyContinuity.ts` from transition data. Used by `useHistoricalData` hook to break constituency vote share chart lines at boundary discontinuities. Rebuild if transition files change.
...
- `wikipedia-mapping.test.ts` - Mapping schema, 100% constituency coverage across all elections
